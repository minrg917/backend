...
추가, 2026-08-27 재설계). 카카오맵 웹사이트가 화면을 그릴 때 쓰는 내부 API를
`httpx`로 직접 호출하는 방식이라 **별도 설치가 필요 없다** — 기본 의존성만으로
동작한다(예전엔 헤드리스 크롬으로 화면을 스크래핑해서 Chrome·Selenium을 서버에
따로 설치해야 했지만, 이제 없어졌다). 실패해도 조용히 꺼진 채로 돌고, 사장님은
3.2로 직접 입력하면 된다.

자세한 설계는 `docs/IMPLEMENTATION.md`의 2026-08-27 항목 참고.
